...
async function createWorkspace(request: any): Promise<string> {
  const res = await request.post('/api/v1/workspaces', { data: { name: 'Run Test WS' } });
...
async function createGraph(request: any, wid: string, nodes: any[], edges: any[]): Promise<any> {
  const res = await request.post(`/api/v1/workspaces/${wid}/graphs`, {
    data: { name: 'run-test-graph', nodes, edges },
  });
  expect(res.status()).toBe(201);
  return res.json();
}
...
test.describe('Graph Runs API', () => {
  ...
  test('POST /runs copies nop nodes and edges as a snapshot', async ({ request }) => {
    const wid = await createWorkspace(request);
    try {
      const graph = await createGraph(
        request, wid,
        [{ node_type: 'nop', name: 'alpha' }, { node_type: 'nop', name: 'beta' }],
      ...
      for (const rn of run.run_nodes) {
        ...
        // nop nodes have no agent config
      ...
      expect(alphaRunNode.node_type).toBe('nop');
  ...
  test('POST /runs on a node with null name preserves null', async ({ request }) => {
    const wid = await createWorkspace(request);
    try {
      const graph = await createGraph(request, wid, [{ node_type: 'nop' }], []);
      const res = await request.post(`/api/v1/workspaces/${wid}/graphs/${graph.id}/runs`);
      expect(res.status()).toBe(201);
      const run = await res.json();
