...
def is_supported_model_for_agent_type(agent_type: str, model: str) -> bool:
    return model in {supported.value for supported in SUPPORTED_MODELS_BY_AGENT_TYPE.get(agent_type, ())}
...
def validate_supported_model_for_agent_type(agent_type: str, model: str) -> str:
    if not is_supported_model_for_agent_type(agent_type, model):
        allowed = ", ".join(item.value for item in SUPPORTED_MODELS_BY_AGENT_TYPE.get(agent_type, ()))
        raise ValueError(f"Model '{model}' is not supported for agent type '{agent_type}'. Allowed: {allowed}")
    return model
